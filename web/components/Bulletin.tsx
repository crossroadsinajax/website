...
const BulletinItem: React.FC<{ bulletinItem: BulletinItemProp }> = ({
  ...
  if (bulletinItem.contactEmail || bulletinItem.contactPhone) {
    cardText.push(" at ")
    cardText.push(
      <a href={`mailto:${bulletinItem.contactEmail}`}>
        {" "}
        {bulletinItem.contactEmail}{" "}
      </a>
    )
  ...
  return (
    <div className="card">
      <div className="card-body">
        ...
        {bulletinItem.date && bulletinItem.date !== "" && (
          <h6 className="card-subtitle">{bulletinItem.date}</h6>
        )}
        <p
          className="card-text"
          dangerouslySetInnerHTML={{ __html: bulletinItem.body }}
        ></p>
      </div>
...
const BulletinSection: React.FC<{
  ...
  return (
    <div>
      <a
        href="#collapseBulletin"
        data-toggle="collapse"
        role="button"
        aria-controls="collapseBulletin"
        style={{ color: "black" }}
      >
        <h2>{section.title}</h2>
      </a>
      <div id="collapseBulletin" className="panel-collapse">
        <div className="card-columns">{itemsTpl}</div>
      </div>
    </div>
  )
}
...
export const Bulletin: React.FC<{
  ...
  return <BulletinSection section={bulletinSection} />
}
